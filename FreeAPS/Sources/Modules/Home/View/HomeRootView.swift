...
extension Home {
    struct RootView: BaseView {
        ...
        @FetchRequest(
            entity: LastLoop.entity(),
            sortDescriptors: [NSSortDescriptor(key: "timestamp", ascending: false)]
        ) var enactedLoop: FetchedResults<LastLoop>
        ...
        var carbsAndInsulinView: some View {
            HStack {
                if let settings = state.settingsManager {
                    ...
                    let previousLoop = (state.suggestion?.reasonParts.count ?? 0) <= 1 ? enactedLoop.first : nil
                        ...
                        HStack(spacing: 0) {
                            if let loop = state.suggestion, let cob = loop.cob {
                                Text(numberFormatter.string(from: cob as NSNumber) ?? "0")
                                    .font(.statusFont).bold()
                                // Display last loop, unless very old (30 min)
                            } else if let lastLoop = previousLoop,
                                      abs((lastLoop.timestamp ?? .distantPast).timeIntervalSinceNow.minutes) < 30
                            {
                                let cob = (lastLoop.cob ?? 0) as Decimal
                                Text(
                                    numberFormatter.string(from: cob as NSNumber) ?? "0"
                                ).font(.statusFont).bold()
                            } else {
                            ...
                            if let loop = state.suggestion, let iob = loop.iob {
                                Text(
                                    numberFormatter.string(from: iob as NSNumber) ?? "0"
                                      ...
                                      abs((lastLoop.timestamp ?? .distantPast).timeIntervalSinceNow.minutes) < 30
                            {
                                let iob = (lastLoop.iob ?? 0) as Decimal
                                Text(
                                    numberFormatter.string(from: iob as NSNumber) ?? "0"
                                ).font(.statusFont).bold()
                            } else {
