...
class BlePodComms: PodComms {
    ...
    ///     [9] AidPodStatus: G3.11 (Gen1) or UnifiedAidPodStatus: G3.12 (non-Gen1)
    ...
    private func o5PreSetupSteps(blePodMessageTransport: BlePodMessageTransport, majorVersion: Int?) throws {
        ...
        try o5SendAidSetupCommands(transport: blePodMessageTransport, majorVersion: majorVersion)
        saveState()
    ...
    ///   9. AidPodStatusCommand — G3.11 (majorVersion < 7), else UnifiedAidPodStatusCommand — G3.12
    private func o5SendAidSetupCommands(transport: BlePodMessageTransport, majorVersion: Int?) throws {
        ...
        let useGen1AidPodStatus = majorVersion.map { o5PodIsV1(majorVersion: $0) } ?? false
        if let majorVersion = majorVersion {
            log.info("O5 AID [9/9]: majorVersion=%{public}d, using %{public}@",
                     majorVersion, useGen1AidPodStatus ? "AidPodStatusCommand (G3.11)" : "UnifiedAidPodStatusCommand (G3.12)")
        } else {
            log.info("O5 AID [9/9]: majorVersion unavailable, defaulting to UnifiedAidPodStatusCommand (G3.12)")
        }

        if skipO5AID9 {
            /// JPM seems to fail with an emptyValue trying to read response, just skip for now?
            ...
            do {
                if useGen1AidPodStatus {
                    ...
                    let response = try transport.sendO5AidCommand(payload, responsePrefix: prefix)
                    log.info("O5 AID [9/9]: AidPodStatus response: %{public}d bytes — %{public}@",
        ...
        log.info("O5 AID: AID setup command exchanges complete")
    }
    ...
    /// VersionResponse instead (observed for the pre-SetupPod GetStatus).
            ...
            do {
                ...
                if self.podState!.setupProgress.isPaired == false && self.podType.isO5 {
                    ...
                    if !pairPodRanGetPodVersion {
                        ...
                        o5MajorVersion = Int(versionResponse.firmwareVersion.major)
                        ...
                        o5MajorVersion = self.parseMajorVersion(from: self.podState?.firmwareVersion)
                    }

                    if let o5MajorVersion = o5MajorVersion {
                        self.log.info("O5: Using firmware majorVersion=%{public}d for AID command selection", o5MajorVersion)
                    } else {
                        self.log.info("O5: Firmware majorVersion unavailable; default AID command selection will be used")
                    }

                    self.log.info("O5: Running pre-SetupPod intermediate steps (AID commands)")
                    try self.o5PreSetupSteps(blePodMessageTransport: preSetupTransport, majorVersion: o5MajorVersion)
