...
struct PairPodView: View {
    ...
    var body: some View {
                    ...
                    InstructionList(instructions: self.viewModel.podType.usesRileyLink ?
                        [
                            String(format: LocalizedString("Fill a new %1$@ pod with U-100 Insulin (leave %2$@ needle cap on). Listen for 2 beeps.",
                                ...
                                self.viewModel.podType.name, self.viewModel.podType.tabColor),
        ...
        .navigationBarTitle(String(format: LocalizedString("Pair %1$@ Pod", comment: "Title for pod pairing screen (1: pod type brief name)"), self.viewModel.podType.briefName) /* ZZZ , displayMode: .automatic ZZZ */)
